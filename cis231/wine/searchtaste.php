...
if (isset($_GET['tasteID'])) {
  $colname_Recordset1 = $_GET['tasteID'];
}
mysql_select_db($database_School, $School);
$query_Recordset1 = sprintf("SELECT wine.wineID, wine.wineName, wine.wineDesc, location.locationName, location.locationID, wine.catName, winetaste.tasteName, MONTHNAME(wine.lastUpdated), DAY(wine.lastUpdated), YEAR(wine.lastUpdated) FROM wine INNER JOIN location on wine.locationID = location.locationID INNER JOIN winetaste ON wine.tasteID = winetaste.tasteID WHERE wine.tasteID = %s ORDER BY wineName ASC", GetSQLValueString($colname_Recordset1, "int"));
...
<title>Search by Taste Results</title>
<link href="winelist.css" rel="stylesheet" type="text/css">
</head>

<body>
<?php require dirname(__FILE__).'/header.php' ?>
<h1>Wine List for <?php echo $row_Recordset1['tasteName']; ?> Wines</h1>
<table id='winerylist'>
  <tr>
